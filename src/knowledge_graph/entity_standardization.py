...
def standardize_entities(triples: List[Dict], config: Dict) -> List[Dict]:
    ...
    logger.info("Standardizing entity names across all triples...")
    ...
    for i, triple in enumerate(triples):
        ...
        if not (isinstance(triple, dict) and
                "subject" in triple and "predicate" in triple and "object" in triple and
            ...
            invalid_count += 1
            continue
        # Ensure chunk info keys exist, even if None/empty initially (will be preserved)
        triple.setdefault('chunk_id', None)
        triple.setdefault('chunk_text', "")
        # Preserve entity types if they exist from extraction
    ...
    if not valid_triples:
        logger.error("No valid triples found for entity standardization.")
        return []
    ...
    def normalize_text(text):
        text = text.lower()
        # More comprehensive stopwords, consider domain specifics
        stopwords = {"the", "a", "an", "of", "and", "or", "in", "on", "at", "to", "for", "with", "by", "as", "ltd", "inc", "corp", "limited"}
        # Keep alphanumeric and basic separators like '-' or '#' if relevant
    ...
    for group_key, variants_lower in entity_groups.items():
        if len(variants_lower) == 1:
            # Only one variant, use its most common original casing
            lower_variant = variants_lower[0]
            # Find the most frequent original casing for this single variant
            original_casing = max(entity_details[lower_variant]["casings"], key=len) if entity_details[lower_variant]["casings"] else lower_variant
            ...
            # Sort variants by length (descending), then by count (descending)
            standard_form_lower = sorted(variants_lower, key=lambda x: (-len(x), -entity_details[x]["count"]))[0]

            # Get the original casing for the chosen standard form (prefer longest casing)
            standard_form_original = max(entity_details[standard_form_lower]["casings"], key=len) if entity_details[standard_form_lower]["casings"] else standard_form_lower

            logger.debug(f"Group '{group_key}': Variants {variants_lower} -> Chose '{standard_form_original}' (from '{standard_form_lower}') as standard.")
    ...
    # 4. Second pass: check for root word/subset relationships (Optional, review if needed)
    # This pass attempts to merge different standard forms identified above.
    # CAUTION: This can sometimes merge unrelated entities if not careful.
    # Consider disabling or refining if it causes issues. For now, keep original logic.
    ...
    for i, entity1_orig in enumerate(sorted_standards_original):
        ...
        for entity2_orig in sorted_standards_original[i + 1:]:
            ...
            if e1_lower == e2_lower: continue # Skip if already same standard form

            # --- Check for potential type mismatch before merging ---
            types1 = entity_details[e1_lower]["types"]
            types2 = entity_details[e2_lower]["types"]
            # Only consider merging if types overlap OR one has no type info
            if types1 and types2 and not types1.intersection(types2):
                 ...
                 continue # Don't merge if types are known and different

            e2_words = set(e2_lower.split())
            # Check for subset relationship
            if e1_words.issubset(e2_words) and len(e1_words) > 0:
                # Prefer the shorter, more general term as standard in subset cases? Or longest?
                # Let's stick to LONGEST for consistency with the primary rule for now.
                additional_standardizations[e1_lower] = entity2_orig # Map shorter lower -> longer original
            elif e2_words.issubset(e1_words) and len(e2_words) > 0:
                additional_standardizations[e2_lower] = entity1_orig # Map shorter lower -> longer original
            else:
                # Check for stem similarity (more complex, keep as is for now)
                stems1 = {word[:4] for word in e1_words if len(word) > 4}
                stems2 = {word[:4] for word in e2_words if len(word) > 4}
                if stems1 and stems2: # Avoid division by zero
                    ...
                    if min_stems > 0 and shared_stems and (len(shared_stems) / min_stems) > 0.5:
                        # If stems are similar, prefer the LONGEST original form as standard
    ...
    # Apply additional standardizations to the map (resolve chains if necessary)
    final_map = standardized_entities_map.copy()
    changed = True
    while changed:
        changed = False
        for entity_lower, standard_orig in list(final_map.items()): # Iterate over copy
            standard_lower = standard_orig.lower()
            # If the current standard form itself has a further standardization...
            if standard_lower in additional_standardizations:
                new_standard = additional_standardizations[standard_lower]
                # Update the mapping if it's different
    ...
    for triple in valid_triples:
        ...
        # Get the final standard names
        final_subj = final_map.get(subj_lower, triple["subject"])
        final_obj = final_map.get(obj_lower, triple["object"])

        # --- Get consolidated types for the final entities ---
        # Get types associated with the final standard name's lowercase version
        final_subj_types = entity_details.get(final_subj.lower(), {}).get("types", set())
        final_obj_types = entity_details.get(final_obj.lower(), {}).get("types", set())
        # Use original types as fallback if no types were associated with the final standard form
        subj_type = next(iter(final_subj_types), triple.get("subject_type"))
        obj_type = next(iter(final_obj_types), triple.get("object_type"))


        # Create the new triple dictionary
        new_triple = {
            "subject": final_subj,
            "subject_type": subj_type, # Add consolidated type
            "predicate": limit_predicate_length(triple["predicate"]),
            "object": final_obj,
            "object_type": obj_type, # Add consolidated type
        ...
        # logger.debug(f"Standardized triple: {new_triple}") # Keep logging minimal
    ...
    if config.get("standardization", {}).get("use_llm_for_entities", False):
        ...
        # This function needs modification to preserve chunk_id/text/types if enabled
    ...
    logger.info(f"Standardized entities. Final triple count: {len(filtered_triples)}")
    if filtered_triples:
        logger.debug(f"Example standardized triple with chunk info & types: {filtered_triples[0]}")
