...
class Neo4jExporter:
    ...
    def store_triples(self, triples: List[Dict], store_chunks: bool = True, sanitize: bool = True):
        """
        ...
        Args:
            triples: A list of dictionaries, each representing a triple.
                     ...
                     ASSUMES 'subject' and 'object' contain standardized names.
        ...
        # Use a set for the cache for slightly faster lookups
        processed_chunk_ids: Set[str] = set()

        logger.info("Starting batch transaction to store %d triples...", len(triples))
        try:
            ...
            with self.driver.session(database="neo4j") as session: # Specify database if needed
                # Pass parameters to the transaction function
            ...
            logger.info("Batch transaction for storing triples completed.")

        except Exception as e:
            logger.error("Batch transaction failed: %s", e, exc_info=True)
    ...
    def _store_triples_transaction(self, tx: Transaction, triples: List[Dict], processed_chunk_ids: Set[str], store_chunks: bool, sanitize: bool):
        ...
        for i, triple in enumerate(triples):
            ...
            # --- Cypher Query for Nodes and Relationship ---
            # Using dynamic labels in MERGE
            cypher_rel = f"""
            MERGE ({subj_pattern} {{name: $subject}})
            MERGE ({obj_pattern} {{name: $object}})
            // MERGE relationship with sanitized type using backticks
            ...
            ON MATCH SET
                // Optionally update properties on match, e.g., update confidence if higher?
                // For now, just update confidence and inferred status
                r.confidence = $confidence,
                r.inferred = $inferred,
            ...
            try:
                tx.run(
                    ...
                    subject=subject,
                    object=object_,
                    original_predicate=predicate, # Store original predicate text
                    inferred=inferred,
